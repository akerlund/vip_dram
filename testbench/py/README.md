...
TLM and advances simulation time itself with cocotb `Timer`s; `tb/dram_hdl_top.sv`
...
python3 run_all_tcs.py tc_dram_smoke tc_dram_read_fault
```
...
`cocotb_module: dram_tb_top`). The wrapper puts `tb/` on `PYTHONPATH` (the top
...
- **`tb/dram_tb_top.py`** — the cocotb top (counterpart of `../sv/tb/dram_tb_top.sv`).
  ...
  exposes one public `@cocotb.test` name `tc_dram_*` per TC that calls
  `run_test("tc_dram_<name>")`.
- **`tc/tc_dram_*.py`** — the 15 ported `uvm_test` cases (1:1 with `../sv/tc/`).
- **`tb/`** — env, TLM driver, scoreboard, and the `dram_hdl_top.sv` shell.
